...
* **受講生コース情報**
  ...
  * **ユーザーの手間とミスを減らす日付の自動計算ロジック**: 受講期間を1年間とする仕様に基づき、コース開始日時（`courseStartAt`）が入力された際、システム側で自動的にその1年後を計算してコース終了日時（`courseEndAt`）に設定するビジネスロジックを実装しました。Java 8以降の推奨APIである `LocalDateTime` を活用し、手動入力による整合性エラー（開始日より前の日付が終了日に入力される等）を未然に防ぐ堅牢な設計にしています。
